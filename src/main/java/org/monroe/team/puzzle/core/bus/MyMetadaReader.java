package org.monroe.team.puzzle.core.bus;
...
public class MyMetadaReader extends MetadataReader{
        ...
        for (int i = 0; i < length; i++) {
            ...
            if (!ReflectionUtils.containsOverridingMethod(allHandlers, handler)) {
                ...
                if (messageClasses.length ==1 && messageClasses[0] == Event.class) {
                    try {
                        messageClasses[0]= (Class) ((ParameterizedType) target.getGenericSuperclass()).getActualTypeArguments()[0];
                    } catch (Exception e) {
